    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="underGoods(props.item)">下架</v-btn>
          <v-btn icon v-else @click="underGoods(props.item)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      underGoods(oldGoods){
        var str = oldGoods.saleable ? "下架":"上架"
        this.$message.confirm("确认要"+str+"该商品吗？")
          .then(() => {
            this.$http.post("/item/spu/" + oldGoods.id)
              .then(() => {
                // 重新加载数据
                this.getDataFromServer();
